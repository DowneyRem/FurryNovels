...
from Translate import getLanguage, getLangSystem, translate, transWords, transPath
from config import novel_path, testMode
...
class PixivSeries(PixivBase):
	...
	all_novels_list = list[int]()  # 系列所有小说的 ID
	...
	def getNovelsList(self, force_update=False) -> None:
		...
		def addList(json: any):
			for novel in json.novels:
				self.all_novels_list.append(novel.id)
				if novel.visible:
		...
		if len(self.all_novels_list) >= 30:  # 1次最多可请求到30个id
